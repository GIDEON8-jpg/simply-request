...
-- Fix ALL tables: convert RESTRICTIVE policies to PERMISSIVE
...
CREATE POLICY "Anyone can view active suppliers"
  ...
  USING (status = 'active'::supplier_status);
...
CREATE POLICY "Admin and CEO can view all payments"
  ON public.payments FOR SELECT TO authenticated
  USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'ceo'::app_role));
...
DROP POLICY IF EXISTS "HR can create requisitions for their department" ON public.requisitions;
...
CREATE POLICY "Preparers can create requisitions"
  ON public.requisitions FOR INSERT TO authenticated
  WITH CHECK (has_role(auth.uid(), 'preparer'::app_role) AND department = (SELECT department FROM profiles WHERE id = auth.uid()));

CREATE POLICY "HODs can create requisitions"
  ON public.requisitions FOR INSERT TO authenticated
  WITH CHECK (has_role(auth.uid(), 'hod'::app_role) AND department = (SELECT department FROM profiles WHERE id = auth.uid()));

CREATE POLICY "Technical Directors can create requisitions"
  ON public.requisitions FOR INSERT TO authenticated
  WITH CHECK (has_role(auth.uid(), 'technical_director'::app_role) AND department = (SELECT department FROM profiles WHERE id = auth.uid()));

CREATE POLICY "CEOs can create requisitions"
  ON public.requisitions FOR INSERT TO authenticated
  WITH CHECK (has_role(auth.uid(), 'ceo'::app_role) AND department = (SELECT department FROM profiles WHERE id = auth.uid()));

CREATE POLICY "HR can create requisitions"
  ON public.requisitions FOR INSERT TO authenticated
  WITH CHECK (has_role(auth.uid(), 'hr'::app_role) AND department = (SELECT department FROM profiles WHERE id = auth.uid()));
