...
-- Drop ALL existing RESTRICTIVE policies on department_budgets and recreate as PERMISSIVE
...
-- Drop ALL existing RESTRICTIVE policies on requisitions and recreate as PERMISSIVE
DROP POLICY IF EXISTS "Preparers can create requisitions" ON public.requisitions;
DROP POLICY IF EXISTS "HODs can create requisitions" ON public.requisitions;
...
DROP POLICY IF EXISTS "HR can create requisitions" ON public.requisitions;
...
-- Drop ALL existing RESTRICTIVE policies on suppliers and recreate as PERMISSIVE
...
CREATE POLICY "HR can manage suppliers"
...
-- Drop ALL existing RESTRICTIVE policies on audit_logs and recreate as PERMISSIVE
...
-- Drop ALL existing RESTRICTIVE policies on profiles and recreate as PERMISSIVE
...
-- Drop ALL existing RESTRICTIVE policies on user_roles and recreate as PERMISSIVE
...
-- Drop ALL existing RESTRICTIVE policies on payments and recreate as PERMISSIVE
...
DROP POLICY IF EXISTS "Admin and CEO can view all payments" ON public.payments;

CREATE POLICY "Authorized can view payments"
...
-- Drop ALL existing RESTRICTIVE policies on tax_clearances and recreate as PERMISSIVE
...
CREATE POLICY "Anyone can view tax clearances"
  ON public.tax_clearances FOR SELECT TO authenticated
  USING (true);
...
CREATE POLICY "HR can upload tax clearances"
  ON public.tax_clearances FOR INSERT TO authenticated
  WITH CHECK (has_role(auth.uid(), 'hr'::app_role));

-- Drop ALL existing RESTRICTIVE policies on requisition_documents and recreate as PERMISSIVE
